...
if 'darwin' in sys.platform:
    ...
    # path_tardis = opj('Users', 'alexandernitsch', 'hummel_mount', 'Data')
    path_hummel_shared = opj('/Users', 'alexandernitsch', 'hummel_mount', 'Data')
    path_hummel_own = opj('/Users', 'alexandernitsch', 'hummel_mount_nitsch', 'RETREAT2026') 
    ...
    path_hummel_shared = opj('/beegfs', 'g', 'schuck', 'bbf2463', 'Projects', 'Retreat2026', 'Data')
    path_hummel_own = opj('/beegfs', 'u', 'bbf2188', 'RETREAT2026') 

    # # path to the project root:
    # project_name = 'highspeed-decoding'
    # # path_root = os.getenv('PWD').split(project_name)[0] + project_name
    # # path_root = opj('beegfs', 'g', 'schuck', 'bbf2463', 'Projects', 'Retreat2026', project_name)
    # b_number = 'bbf2463'
    # path_root = opj('/beegfs', 'u', b_number, 'RETREAT2026', project_name) 
    # # define the path to the cluster:
    # path_tardis = path_root
    # # define the path to the server:
    # path_server = opj('/beegfs', 'g', 'schuck', 'bbf2463', 'Projects', 'Retreat2026', project_name)
...
========================================================================
'''
path_decoding = opj(path_hummel_own, 'decoding')
...
path_events = opj(path_hummel_shared, 'highspeed-bids', sub, 'ses-*', 'func', '*tsv')
os.chdir(opj(path_hummel_shared, 'highspeed-bids'))
...
path_fmriprep = opj(path_hummel_shared, 'highspeed-fmriprep', 'fmriprep', sub)
path_level1 = opj(path_hummel_shared, 'highspeed-glm', 'l1pipeline')
path_masks = opj(path_hummel_shared, 'highspeed-masks', 'masks')
...
# path_rest = opj(path_hummel_shared, 'masks', 'masks', 'smooth', sub, '*', '*task-rest*nii.gz')
...
# load visual mask:
mask_vis = image.load_img(path_mask_vis_task[0]).get_fdata().astype(int)
logging.info('successfully loaded one visual mask file!')
# load tmap data:
tmaps = [image.load_img(i).get_fdata().astype(float) for i in copy.deepcopy(path_tmap)]
logging.info('successfully loaded the tmaps!')
# load hippocampus mask:
mask_hpc = [image.load_img(i).get_fdata().astype(int) for i in copy.deepcopy(path_mask_hpc_task)]
logging.info('successfully loaded one visual mask file!')
'''
FEATURE SELECTION
'''
...
tmaps_masked_thresh = [image.load_img(i).get_fdata().astype(float) for i in tmaps_masked_thresh_img]
...
masks_final = [image.new_img_like(path_func_task[0], i.astype(int)) for i in copy.deepcopy(tmaps_masked_thresh_bin)]
...
# create a dictionary with all values as independent instances:
# see here: https://bit.ly/2J1DvZm
clf_set = {key: OneVsRestClassifier(LogisticRegression(
            C=1., 
            penalty='l2', 
            solver='lbfgs',
            max_iter=4000, 
            class_weight='balanced', 
            random_state=42)) for key in class_labels} # multi_class='ovr'
classifiers = {
    'log_reg': LogisticRegression(
                                C=1., 
                                solver='lbfgs', 
...
'''
========================================================================
SAVE WHOLE-BRAIN SINGLE TRIAL (TR) NIFTIS FOR CLASSIFIER TRAINING:
========================================================================
'''
...
def detrend_img(data, t_tr=1.25):
    data_detrend = clean_img(
        imgs=data, t_r=t_tr, detrend=True, standardize=False)
    return data_detrend

data_detrend_run = [detrend_img(i) for i in data_task]
data_detrend_runs = concat_imgs(data_detrend_run)

data_detrend_runs_trs = nilearn.image.index_img(data_detrend_runs, train_odd_peak.trs)


run_list_z = []

for run in runs: 
    tmp = nilearn.image.index_img(data_detrend_runs_trs, train_odd_peak.runs==run) 
    tmp = clean_img(imgs=tmp, t_r=t_tr, detrend=False, standardize=True)
    run_list_z.append(tmp)
    
data_zscored = concat_imgs(run_list_z)

for i_tr, tr in enumerate(train_odd_peak.trs):
    cur_img = nilearn.image.index_img(data_zscored, i_tr)  
    fn = opj(path_out_data, f'{sub}_run-{train_odd_peak.runs[i_tr]:02}_trial-{train_odd_peak.trials[i_tr]:02}_stim-{train_odd_peak.stim[i_tr]}.nii.gz')
    cur_img.to_filename(fn)
